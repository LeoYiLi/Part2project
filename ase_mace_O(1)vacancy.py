...
atoms0 = read("YBa2Cu3O7_Ovac1.cif")
...
def enforce_a_equals_b(atoms):
    a, b, c, alpha, beta, gamma = atoms.cell.cellpar()
    ab = 0.5*(a + b)
    newcell = cellpar_to_cell([ab, ab, c, 90.0, 90.0, 90.0])
    atoms.set_cell(newcell, scale_atoms=True)

dyn = BFGS(ucf, logfile="-", trajectory="vc.traj", maxstep=0.02)

dyn.attach(enforce_a_equals_b, interval=5, atoms=atoms0)
dyn.run(fmax=0.005, steps=1000)
'''

dyn = BFGS(atoms0, trajectory="vc.traj", logfile="-")
dyn.run(fmax=0.005, steps=1000)
write("YBa2Cu3O7_Ovac1_relaxed_MACE.cif", atoms0)
...
path = "YBa2Cu3O7_Ovac1_relaxed_MACE.cif"
